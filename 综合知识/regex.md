##正则表达式
[正则表达式在线工具](http://www.atool.org/regex.php) 
##常用正则表达式 
###一、校验数字的表达式
...
32 中国邮政编码：[1-9]\d{5}(?!\d) (中国邮政编码为 6 位数字) 33 IP 地址：\d+\.\d+\.\d+\.\d+ (提取 IP 地址时有用) 34 IP 地址：((?:(?:25[0-5]|2[0-4]\\d|[01]?\\d?\\d)\\.){3}(?:25[0-5]|2[0-4]\\d|[01]?\\d?\\d))
```
